...
if __name__ == "__main__":
    ...
    testSetSizes = [ts/100 for ts in range(90, 9, -5)]  # testset sizes
    ...
    resultsPath = "results/rq-trainset_size/{}".format(thresholdName)
    if not os.path.exists("results"):
        os.makedirs("results")
    if not os.path.exists("results/rq-trainset_size/"):
        os.makedirs("results/rq-trainset_size")
    ...
    for projectName in projectList:
        ...
        for testSetSize in testSetSizes:
            ...
            print()
            print()
            print(projectName.upper())
            print("TESTSET SIZE:", testSetSize)

            resListDict = defaultdict(list)

            kf = StratifiedShuffleSplit(n_splits=numSplits, test_size=testSetSize, random_state=42)
            successFold = 0
            ...
            if len(resListDict) == 0:
                print("resListDict size: 0")
                continue

            outputFile = "{}/{}_TestSetSize-{}.csv".format(resultsPath, projectName.replace("/", "_"), testSetSize)
        ...
        p1 = time.time()
        print("Running time for test size:", p1 - p0)
